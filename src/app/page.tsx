...
import { Search, ShoppingBag, Sparkles, Shield, X, Plus, Minus, Trash2 } from "lucide-react";
              ...
              <button
                ...
                className={`px-5 py-1.5 text-xs font-bold rounded-full border transition-all cursor-pointer ${
                  (cat === "All" ? selectedCategory === "All" : selectedCategory === cat)
                    ...
                    : "bg-white text-[#5D4037] border-[#E7E0D9] hover:bg-white/80"
                }`}
              >
        ...
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="absolute inset-y-0 right-0 max-w-md w-full bg-white shadow-2xl flex flex-col justify-between border-l border-[#EFEBE9]">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {cart.length === 0 ? (
                <div className="text-center py-20 text-[#A1887F] space-y-2">
                  <div className="text-4xl">🧁</div>
                  <p className="text-sm font-medium">Your bakery selection box is currently empty.</p>
                </div>
              ) : (
                cart.map((item) => (
                  <div key={item.product.id} className="flex gap-4 border border-[#FAF7F2] p-3 rounded-xl bg-white shadow-2xs">
                    <img src={item.product.img} alt={item.product.name} className="w-16 h-16 object-cover rounded-lg" />
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <h4 className="text-sm font-bold text-[#4E342E]">{item.product.name}</h4>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center border border-[#E7E0D9] rounded-md overflow-hidden bg-white">
                          <button onClick={() => updateQuantity(item.product.id, -1)} className="p-1 text-[#8D6E63] hover:bg-[#FAF7F2] cursor-pointer">
                            <Minus className="w-3 h-3" />
                          </button>
                          <span className="px-2.5 text-xs font-bold text-[#4E342E]">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.product.id, 1)} className="p-1 text-[#8D6E63] hover:bg-[#FAF7F2] cursor-pointer">
                            <Plus className="w-3 h-3" />
                          </button>
                        </div>
                        <button onClick={() => removeFromCart(item.product.id)} className="text-red-400 hover:text-red-600 transition-colors cursor-pointer p-1">
            ...
            {cart.length > 0 && (
              <div className="p-6 border-t border-[#FAF7F2] space-y-4 bg-[#FAF7F2]/40">
                <div className="flex justify-between items-center text-[#4E342E]">
                  <span className="text-sm font-bold">Subtotal Due:</span>
                  <span className="text-2xl font-black">${cartTotal.toFixed(2)}</span>
                </div>
                <button
                  onClick={() => alert("Simulated checkout processed successfully! Thanks for supporting Cake-On-Cake Bakery.")}
                  className="w-full bg-[#8D6E63] hover:bg-[#795548] text-white py-3.5 rounded-xl font-bold text-sm shadow-sm transition-colors text-center block cursor-pointer"
                >
